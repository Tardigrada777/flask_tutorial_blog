...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Поздравляем, регистрация прошла успешно', category='message')
        return redirect(url_for('login'))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('На вашу почту отправлена инструкция по сбросу пароля')
        return redirect(url_for('login'))
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash('Ваш пароль был успешно сброшен')
        return redirect(url_for('login'))
...
def follow(username):
    ...
    db.session.commit()
    flash('Поздравляем, Вы подписались на {}'.format(username))
    return redirect(url_for('user', username=username))
